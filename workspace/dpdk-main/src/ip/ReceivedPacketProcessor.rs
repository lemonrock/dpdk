...
// TODO: IPv6 options...
// TODO: IPv4 TOS / https://en.wikipedia.org/wiki/Type_of_service and other options... //www.iana.org/assignments/internet_protocol-parameters/internet_protocol-parameters.xhtml
...
{
	#[inline(always)]
	fn vlanPacket(&mut self, packet: *mut rte_mbuf, offsetToJustAfterOurEtherTypeOfVlan: u32, vlanQinQId: Option<VirtualLanIdentifier>, sourceEthernetAddress: *const ether_addr)
	{
		#[macro_export]
		macro_rules! common
		{
			...
			{
				{
					...
					let virtual_lan_id = discardPacketIfDropEligibleBitSetOrTagControlInformationIsInvalid!($packet, tagControlInformation);
					let nextOffset = $ourOffset + SizeOfEtherType;
					(nextOffset, ($vlanQinQId, virtual_lan_id))
				}
			}
		}
		...
		match etherType!(packet, ourOffset)
		{
			ETHER_TYPE_IPv6 =>
			{
				let (nextOffset, virtual_lan_key) = common!(packet, offsetToJustAfterOurEtherTypeOfVlan, ourOffset, vlanQinQId);
				let ipState = discardPacketIfNone!(packet, self.destinations.ipState(virtual_lan_key));
	...
	fn qInQPacket(&mut self, packet: *mut rte_mbuf, sourceEthernetAddress: *const ether_addr)
	{
		...
		let virtual_lan_id = discardPacketIfDropEligibleBitSetOrTagControlInformationIsInvalid!(packet, tagControlInformation);
		self.vlanPacket(packet, OffsetToJustAfterVlanHeaderEtherTypeIfVlanQinQ, virtual_lan_id, sourceEthernetAddress)
	}
}
