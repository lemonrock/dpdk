...
pub struct IpV4State
{
	...
	pub ourIpV4Addresses: HashMap<IpV4HostAddress, IpAddressInformation>,
...
impl IpV4State
{
	...
	pub fn ipV4Packet(&mut self, packet: *mut rte_mbuf, layer2HeaderLength: u32)
	{
		...
		let (layer3Length, layer3HeaderLength, layer4Protocol, layer4HeaderLength) = mutatePacketData!(packet, layer2HeaderLength, ipv4_hdr, ipV4Header,
		{
			...
			discardPacketIf!(packet, senderIpV4Address.isNotValidUnicast());
			...
			else
			{
				...
				let ipAddressInformation = discardPacketIfNone!(packet, self.ipV4State.ourIpV4Addresses(destinationIpV4Address)).unwrap();
		...
		if let Some(ipAddressInformation) = self.ipV4State.ourIpV4Addresses(destinationIpV4Address)
		{
			...
			{
				Layer4Protocol::Tcp => ipAddressInformation.tcpReceiveBurstBuffer.bufferAndSendToTldkWhenFull(packet),
				Layer4Protocol::Udp => ipAddressInformation.udpReceiveBurstBuffer.bufferAndSendToTldkWhenFull(packet),
			}
		}
	}
	...
	pub fn arpPacket(&mut self, packet: *mut rte_mbuf, layer2HeaderLength: u32, sourceEthernetAddress: *const ether_addr, outboundQueue: &mut TransmitBurstQueue)
	{
		macro_rules! extractAddresses
		{
			...
			{
				{
					const SizeOfIpV4Address: u8 = SizeOfIpV4HostAddress as u8;
					
					discardPacketIf!($packet, HardwareType::isNotEthernet2($arpHeader.arp_hrd));
					...
					discardPacketIf!($packet, senderIpV4Address.isNotValidUnicast());
					discardPacketIf!($packet, targetIpV4Address.isNotValidUnicast());
					discardPacketIf!($packet, self.isBlackListedSourceIpV4Address(senderIpV4Address));
		...
		let enqueuePacket =
				...
				mutatePacketData!(packet, layer2HeaderLength, arp_hdr, arpHeaderPointer,
			{
				...
				if likely(operationCode == Operation::REQUEST.toNetworkByteOrder())
				{
					...
					arpHeader.arp_op = Operation::REPLY.toNetworkByteOrder();
				...
				else if likely(operationCode == Operation::REPLY.toNetworkByteOrder())
				{
					let (senderHardwareAddress, senderIpV4Address, targetHardwareAddress, targetIpV4Address) = extractAddresses!(self, packet, arpHeader, arpIpV4Data, ipState);
	...
	#[inline(always)]
	fn isBlackListedSourceIpV4Address(&self, senderIpV4Address: IpV4HostAddress) -> bool
	{
		self.sourceIpV4AddressBlackList.isIpAddressBlackListed(&senderIpV4Address)
	}
	
	#[inline(always)]
	fn isOneOfOurIpV4Addresses(&self, senderIpV4Address: IpV4HostAddress) -> bool
	{
		self.ourIpV4Addresses.get(&senderIpV4Address).is_some()
	}
	
	#[inline(always)]
	fn isNotOneOfOurIpV4Addresses(&self, destinationIpV4Address: IpV4HostAddress, destinationEthernetAddress: *const ether_addr) -> bool
	{
	...
	#[inline(always)]
	fn getOurEthernetAddressForIpV4Address(&self, possiblyOurIpV4Address: IpV4HostAddress) -> Option<ether_addr>
	{
		self.ourIpV4Addresses.get(&possiblyOurIpV4Address).map(|value| value.ourEthernetAddress.clone())
	}
	
	#[inline(always)]
	fn addOrFreshenArpCacheWith(&mut self, senderIpV4Address: IpV4HostAddress, senderHardwareAddress: ether_addr)
	{
