...
{
	#[inline(always)]
	pub fn tleDestinationTemplate(udpFragmentsAndTcpControlPacketBufferPool: *mut rte_mempool, virtualLanTagging: &VirtualLanTagging, differentiatedServiceCodePoint: DifferentiatedServiceCodePoint, hopLimits: u8, ipProtocol: IpProtocol, layer4Protocol: Layer4Protocol) -> (tle_dest, usize)
	{
		...
		let layer2Length = virtualLanTagging.writeLayer2HeaderData(unsafe { buffer.offset(VlanTaggingOffset) }, ipProtocol.asEtherTypeBigEndian());
		...
		let (layer3Length, trailingAddressBytesLength) = ipProtocol.writeLayer3Header(unsafe { buffer.offset(VlanTaggingOffset + layer2Length as isize) }, differentiatedServiceCodePoint, hopLimits, layer4Protocol);
	...
	{
		let (tleDestinationTemplate, numberOfBytesOfTemplateToMemCopy) = Self::tleDestinationTemplate(udpFragmentsAndTcpControlPacketBufferPool, virtualLanTagging, differentiatedServiceCodePoint, hopLimits, IpProtocol::V4, layer4Protocol);
		Self
		{
			ipV4RoutingTable: ipV4RoutingTable,
			tleDestinationTemplate: tleDestinationTemplate,
			numberOfBytesOfTemplateToMemCopy: numberOfBytesOfTemplateToMemCopy,
		}
	}
