...
impl InternetProtocolVersion6PacketHeader
{
	...
	#[inline(always)]
	pub(crate) fn set_payload_length_including_extension_headers(&mut self, payload_length: u16)
	{
		self.payload_length_including_extension_headers = NetworkByteOrderEndianU16::from_native_byte_order_value(payload_length)
	}
	
	/// Sets the explicit congestion notification.
	#[inline(always)]
	pub(crate) fn set_explicit_congestion_notification(&mut self, explicit_congestion_notification: ExplicitCongestionNotification)
	{
		explicit_congestion_notification.into();
		
		let bottom_two_traffic_class_bits = unsafe { *self.version_and_traffic_class_and_flow_label.get_unchecked(1) } & 0b1100_0000;
		
		let second_byte = bottom_two_traffic_class_bits | (explicit_congestion_notification.into() << 4);
	...
	pub(crate) fn process<'a>(&'a mut self, packet: PacketBuffer, packet_processing: &PacketProcessing<impl PacketProcessingDropObserver>, layer_3_length: u16, ethernet_addresses: &'a EthernetAddresses)
	{
		...
		{
			drop!(InternetProtocolVersion6HeaderIsNot6 { ethernet_addresses, header: self }, packet_processing, packet)
		}
		...
		if cfg!(feature = "drop-ipv6-packets-with-non-zero-flow-label")
		{
			...
			{
				drop!(InternetProtocolVersion6FlowLabelIsNonZero { ethernet_addresses, header: self }, packet_processing, packet)
			}
		}
		
		if unlikely!(packet_processing.is_source_internet_protocol_version_6_address_denied(&self.source_address))
		{
			drop!(InternetProtocolVersion6SourceAddressDenied { ethernet_addresses, header: self }, packet_processing, packet)
		}
		...
		{
			drop!(InternetProtocolVersion6DestinationAddressUnspecified { ethernet_addresses, header: self }, packet_processing, packet)
		}
		
		if unlikely!(destination_internet_protocol_version_6_address.is_documentation())
		{
			drop!(InternetProtocolVersion6DestinationAddressDocumentation { ethernet_addresses, header: self }, packet_processing, packet)
		}
		
		if unlikely!(destination_internet_protocol_version_6_address.is_loopback())
		{
			drop!(InternetProtocolVersion6DestinationAddressLoopback { ethernet_addresses, header: self }, packet_processing, packet)
		}
		...
		let address_support = if destination_ethernet_address.is_valid_unicast()
		{
			...
			{
				drop!(InternetProtocolVersion6UnicastDestinationIsNotUs { ethernet_addresses, header: self }, packet_processing, packet)
			}
		...
		else if let Some(lower_32_bits) = destination_ethernet_address.internet_protocol_version_6_multicast_32_bits()
		{
			...
			{
				None => drop!(InternetProtocolVersion6MulticastAddressIsNotMulticast { ethernet_addresses, header: self }, packet_processing, packet),
				
				Some(Err(parsing_error)) => drop!(InternetProtocolVersion6MulticastAddressIsNotValidMulticast { ethernet_addresses, header: self, parsing_error }, packet_processing, packet),
				
				Some(Ok((_lifetime, scope))) =>
				{
					if unlikely!(scope.is_interface_local_also_known_as_loopback())
					{
						drop!(InternetProtocolVersion6DestinationAddressInterfaceLocal { ethernet_addresses, header: self }, packet_processing, packet)
					}
					
					if unlikely!(destination_internet_protocol_version_6_address.does_not_have_lower_32_bits(lower_32_bits))
					{
						drop!(InternetProtocolVersion6MulticastAddressMismatchesEthernetAddress { ethernet_addresses, header: self }, packet_processing, packet)
					}
					...
					{
						drop!(InternetProtocolVersion6MulticastAddressDenied { ethernet_addresses, header: self }, packet_processing, packet)
					}
		...
		{
			drop!(InternetProtocolVersion6EthernetBroadcastShouldNotOccur { ethernet_addresses, header: self }, packet_processing, packet);
		}
		else
		{
			drop!(InternetProtocolVersion6DestinationWasLoopbackOrDocumentationAddress { ethernet_addresses, header: self }, packet_processing, packet)
		};
	...
	#[inline(always)]
	pub(crate) fn process_extension_headers<'a>(&'a mut self, packet: PacketBuffer, packet_processing: &PacketProcessing<impl PacketProcessingDropObserver>, layer_3_length: u16, ethernet_addresses: &'a EthernetAddresses, address_support: Layer4ProtocolNeedsToSupport)
	{
		...
		loop
		{
			match next_header.into()
			{
				...
				{
					drop!(InternetProtocolVersion6NoNextHeaderIsUnsupported { ethernet_addresses, header: self }, $packet_processing, $packet)
				}
				else
				{
				...
				TransmissionControlProtocol =>
				{
					...
					{
						drop!(InternetProtocolVersion6TransmissionControlProtocolPacketsShouldOnlyBeUnicast { ethernet_addresses, header: self }, packet_processing, packet)
					}
				...
				UserDatagramProtocol =>
				{
					...
					{
						drop!(InternetProtocolVersion6UserDatagramProtocolPacketsMustHaveAChecksumSet { ethernet_addresses, header: self }, packet_processing, packet)
					}
				...
				Routing =>
				{
					...
					{
						drop!(InternetProtocolVersion6RoutingExtensionHeaderRepeated { ethernet_addresses, header: self }, $packet_processing, $packet)
					}
					...
					{
						drop!(InternetProtocolVersion6RoutingExtensionHeaderUnderflow { ethernet_addresses, header: self }, packet_processing, packet)
					}
					...
					match routing_type
					{
						SourceRouteRoutingHeader | NimrodRoutingHeader | Experiment253RoutingHeader | Experiment254RoutingHeader | ReservedRoutingHeader => drop!(InternetProtocolVersion6RoutingExtensionHeaderRoutingTypeIsDeprecatedExperimentalOrReserved { ethernet_addresses, header: self, routing_type, segments_left }, packet_processing, packet),
						
						_ => if unlikely!(segments_left != 0)
						{
							// We do not send an ICMP Parameter Problem, Code 0.
							drop!(InternetProtocolVersion6RoutingExtensionHeaderHasSegmentsLeft { ethernet_addresses, header: self, routing_type, segments_left }, packet_processing, packet)
						},
					}
				...
				Fragment =>
				{
					...
					{
						drop!(InternetProtocolVersion6FragmentExtensionHeaderRepeated { ethernet_addresses, header: self }, packet_processing, packet)
					}
					...
					{
						drop!(InternetProtocolVersion6FragmentExtensionHeaderUnderflow { ethernet_addresses, header: self }, packet_processing, packet)
					}
					...
					if cfg!(feature = "drop-ipv6-fragments-when-first-reserved-field-is-not-zero")
					{
						...
						{
							drop!(InternetProtocolVersion6FragmentExtensionHeaderFirstReservedFieldNonZero { ethernet_addresses, header: self, reserved }, packet_processing, packet)
						}
					}
					...
					let fragment_details =
					{
						...
						if cfg!(feature = "drop-ipv6-fragments-when-second-reserved-field-is-not-zero")
						{
							...
							{
								drop!(InternetProtocolVersion6FragmentExtensionHeaderSecondReservedFieldNonZero { ethernet_addresses, header: self, reserved }, packet_processing, packet)
							}
						}
						...
						if !more_flag_set
						{
							...
							{
								drop!(InternetProtocolVersion6FragmentExtensionHeaderOnlyOneFragmentOrLastFragmentIsFirst { ethernet_addresses, header: self }, packet_processing, packet)
							}
						}
				...
				EncapulatingSecurityPayload => drop!(InternetProtocolVersion6EncapulatingSecurityPayloadExtensionHeaderUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				AuthenticationHeader => drop!(InternetProtocolVersion6AuthenticationHeaderExtensionHeaderUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				InternetControlMessageProtocolIpV6 =>
				{
					if unlikely!(fragment_extension.is_some())
					{
						drop!(InternetProtocolVersion6InternetControlMessageProtocolPacketsShouldNotBeFragmented { ethernet_addresses, header: self }, packet_processing, packet)
					}
				...
				NoNextHeader => drop!(InternetProtocolVersion6NoNextHeaderIsUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				DestinationOptions =>
				{
					if destination_options_count == 2
					{
						drop!(InternetProtocolVersion6MoreThanTwoDestinationOptionsExtensionHeaders { ethernet_addresses, header: self }, packet_processing, packet)
					}
				...
				Mobility => drop!(InternetProtocolVersion6MobilityExtensionHeaderUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				HostIdentityProtocol => drop!(InternetProtocolVersion6HostIdentityProtocolExtensionHeaderUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				Shim6Protocol => drop!(InternetProtocolVersion6Shim6ProtocolExtensionHeaderUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				Experimentation253 | Experimentation254 => drop!(InternetProtocolVersion6ExperimentationExtensionHeaderUnsupported { ethernet_addresses, header: self }, packet_processing, packet),
				
				_ => drop!(InternetProtocolVersion6UnrecognisedExtensionHeaderOrLayer4Protocol { ethernet_addresses, header: self, next_header }, packet_processing, packet)
			}
...
// Is is not an error (RFC 8200 Section 4.5) for a fragment that isn't the first to contain other headers, even though those headers are to be ignored.
macro_rules! process_extension_headers
{
	($self: ident, $packet: ident, $packet_processing: ident, $layer_3_length: ident, $ethernet_addresses: ident) =>
	{
		{
			
			
			// report back:-
			// location of fragment header
			// location (and length) of layer 4 payload
			// ? jumbogram location ?
			// ? fragment size to determine if we should drop ?
		}
	}
}
