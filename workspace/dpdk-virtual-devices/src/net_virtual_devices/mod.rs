// This file is part of dpdk. It is subject to the license terms in the COPYRIGHT file found in the top-level directory of this distribution and at https://raw.githubusercontent.com/lemonrock/dpdk/master/COPYRIGHT. No part of dpdk, including this file, may be copied, modified, propagated, or distributed except according to the terms contained in the COPYRIGHT file.
// Copyright © 2016-2017 The developers of dpdk. See the COPYRIGHT file in the top-level directory of this distribution and at https://raw.githubusercontent.com/lemonrock/dpdk/master/COPYRIGHT.


use super::*;


include!("AfPacketNetVirtualDevice.rs");
include!("BalanceBondingModeTransmitPolicy.rs");
include!("BondingMode.rs");
include!("BondingNetVirtualDevice.rs");
include!("BondingSlave.rs");
include!("cap_u32_to_u31.rs");
include!("cap_u32_to_u31_maximum_power_of_two.rs");
include!("KernelNativeInterfaceNetVirtualDevice.rs");
include!("NetVirtualDevice.rs");
include!("NetVirtualDeviceDriverName.rs");
include!("NetVirtualDeviceName.rs");
include!("NetVSCNetVirtualDevice.rs");
include!("PacketCaptureFileOrInterfacePrefix.rs");
include!("PacketCaptureFileOrInterface.rs");
include!("PacketCaptureNetVirtualDevice.rs");
include!("TapMediaAccessControlAddress.rs");
include!("TapNetVirtualDevice.rs");
include!("TunNetVirtualDevice.rs");
include!("UsefulBondingMode.rs");
include!("VirtIoNetVirtualDevice.rs");
include!("VirtualHostNetVirtualDevice.rs");
