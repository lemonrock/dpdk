...
impl LogicalCore
{
	...
	{
		if unlikely(unsafe { rte_thread_set_affinity(cpu_set) } != 0)
		{
	...
	pub fn add_to_logical_cores_used_as_service_cores(self) -> Result<(), ()>
	{
		...
		else
		{
			match result
			{
				NegativeE::EBUSY | NegativeE::ALREADY => Err(()),
	...
	pub fn start_service_core(self)
	{
		debug_assert!(self.is_role_service(), "Is not a service core");
	...
	{
		if unlikely(unsafe { rte_thread_setname(self.thread(), name.as_ptr()) } != 0)
		{
	...
	pub fn block_until_all_slaves_are_in_the_wait_state()
	{
		...
		{
			slave_logical_core.wait_for_slave()
		}
	}
	...
	pub fn block_until_this_slave_is_in_the_wait_state(self)
	{
		...
		unsafe { rte_eal_wait_lcore(slave_logical_core.into()) };
	}
	...
	{
		&DpdkProcess::global_configuration().lcore_role[self.into()]
	}
	...
	{
		Self::logical_core_global_configuration()[self.into()]
	}
