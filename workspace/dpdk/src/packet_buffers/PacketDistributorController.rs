...
impl PacketDistributorController
{
	...
	#[inline(always)]
	pub fn new(name: &str, numa_numa_choice: NumaNodeChoice, number_of_workers: u8) -> Self
	{
		...
		{
			distributor: unsafe { NonNull::new_unchecked(distributor) },
			number_of_workers,
		}
	}
	...
	///
	/// Ideally, call this from the thread that will use the worker so that memory is created core-local.
	#[inline(always)]
	pub fn worker(self, worker_identifier: u8) -> PacketDistributorWorker
	{
		debug_assert!(worker_identifier < self.number_of_workers, "worker_identifier '{}' equals or exceeds self.number_of_workers", worker_identifier, self.number_of_workers);
		
		PacketDistributorWorker
		{
			distributor: self.distributor,
			worker_identifier: worker_identifier as u32,
		}
	}
