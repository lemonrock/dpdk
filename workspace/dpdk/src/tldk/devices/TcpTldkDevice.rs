...
pub struct TcpTldkDevice(*mut tle_dev);

impl TldkDevice for TcpTldkDevice
{
	...
	{
		let numberAccepted = unsafe { tle_tcp_rx_bulk(self.handle(), pkt, rp, rc, num) };
	...
	{
		unsafe { tle_tcp_tx_bulk(self.handle(), pkt, num) }
	}
}
