...
impl MaximumTransmissionUnitSize
{
	/// Theoretical Minimum.
	///
	/// Unlikely to be be useful or even supported by poll-mode drivers (PMD) or other computers on the network.
	pub const TheoreticalMinimum: Self = from_ethernet_frame_length(EthernetFrameLength::Minimum);
	...
	pub const EthernetV2: Self = from_ethernet_frame_length(EthernetFrameLength::MaximumIncludingCyclicRedundancyCheckWithoutJumboFrames);
	
	/// Maximum value for Jumbo frames.
	pub const PPPoEOverJumboFrames: Self = Self::MaximumJumboValue.decrease_for_pppoe_header();
	
	/// Maximum value for Jumbo frames.
	pub const MaximumJumboValue: Self = from_ethernet_frame_length(EthernetFrameLength::MaximumIncludingCyclicRedundancyCheckWithJumboFrames);
	...
	{
		MaximumTransmissionUnitSize(self.0 - (SizeOfVlanHeader as u16))
	}
	...
	{
		MaximumTransmissionUnitSize(self.0 - ((SizeOfQinQHeader + SizeOfVlanHeader) as u16))
	}
	...
	{
		assert!(bytes >= ETHER_MIN_MTU, "The bytes, '{}', must be greater than ETHER_MIN_MTU ({})", bytes, ETHER_MIN_MTU);
		assert!(bytes <= ETHER_MAX_JUMBO_FRAME_LEN - ETHER_CRC_LEN, "The bytes, '{}', must be less than (ETHER_MAX_JUMBO_FRAME_LEN ({}) - ETHER_CRC_LEN ({}))", bytes, ETHER_MAX_JUMBO_FRAME_LEN, ETHER_CRC_LEN);
