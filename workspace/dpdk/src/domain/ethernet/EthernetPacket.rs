...
macro_rules! parse_802_1ad_virtual_lan_tag_control_information
{
	...
	{
		{
			let (outer_virtual_lan_identifier, outer_class_of_service) = match outer.parse()
			{
				Err(_) => finish!(packet),
			...
			let (inner_virtual_lan_identifier, inner_class_of_service) = match inner.parse()
			{
				Err(_) => finish!(packet),
				Ok((class_of_service, drop_eligible_indicator, outer_virtual_lan_identifier)) =>
				{
			...
			let packet_processing_configuration = match $packet_processing_configuration_by_virtual_lan.get_packet_processing_for_outer_virtual_lan(outer_virtual_lan_identifier, inner_virtual_lan_identifier)
			{
				None => finish!(packet),
...
{
	($packet: ident, $packet_processing_configuration_by_virtual_lan: ident) =>
	{
		{
			let packet_processing_configuration = &packet_processing_configuration_by_virtual_lan.none;
			
			let (source_ethernet_address, destination_ethernet_address) = guard_ethernet_addresses!($packet, packet_processing_configuration);
