...
impl ClassOfService
{
	...
	pub fn equivalent_differentiated_service_code_point(&self) -> DifferentiatedServiceCodePoint
	{
		...
		use self::DifferentiatedServiceCodePoint::*;
		
		match *self
		{
			Background => DefaultForwarding,
			BestEffort => AssuredForwardingClass1LowDropProbability,
			ExcellentEffort => AssuredForwardingClass2LowDropProbability,
			CriticalApplication => AssuredForwardingClass3LowDropProbability,
			Video => AssuredForwardingClass4LowDropProbability,
			Voice => ExpeditedForwarding,
			InterNetworkControl => InterNetworkControl,
