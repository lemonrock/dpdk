...
impl VirtualLanPacketTagControlInformation
{
	/// Parse.
	#[inline(always)]
	pub fn parse(self) -> Result<(ClassOfService, DropEligibleIndicator, Option<VirtualLanIdentifier2>), ()>
	{
		use self::VirtualLanIdentifier::*;
		...
		let virtual_lan_identifier =
		{
			match value & 0x0FFF
			{
				0 => None,
				0xFFF => return Err(()),
				valid @ _ => Some(VirtualLanIdentifier2(valid))
			}
		};
		...
		(class_of_service, drop_eligible_indicator, virtual_lan_identifier)
	}
}
