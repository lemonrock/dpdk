...
pub struct PacketProcessingConfiguration
{
	...
	address_conflict_state: AddressResolutionProtocolAddressConflictState,
}
...
impl PacketProcessingConfiguration
{
	...
	#[inline(always)]
	pub(crate) fn is_ethernet_address_our_valid_unicast_ethernet_address(&self, ethernet_address: &MediaAccessControlAddress) -> bool
	{
		debug_assert!(destination_ethernet_address.is_valid_unicast(), "ethernet_address '{}' is not valid unicast", ethernet_address);
		
		self.our_valid_unicast_ethernet_address == ethernet_address
	}
	
	#[inline(always)]
	pub(crate) fn is_ethernet_address_not_our_valid_unicast_ethernet_address(&self, ethernet_address: &MediaAccessControlAddress) -> bool
	{
		debug_assert!(destination_ethernet_address.is_valid_unicast(), "ethernet_address '{}' is not valid unicast", ethernet_address);
	...
	{
		debug_assert!(destination_ethernet_address.is_valid_unicast(), "source_ethernet_address '{}' is not valid unicast", source_ethernet_address);
	...
	{
		self.address_conflict_state.internet_protocol_version_4_host_address_conflict(packet)
	}
	
	#[inline(always)]
	pub(crate) fn add_to_address_resolution_cache(&self, source_ethernet_address: &MediaAccessControlAddress, internet_protocol_version_4_host_address: InternetProtocolVersion4HostAddress)
	{
