...
impl PrintInformation for Service
{
	...
	{
		if unlikely(unsafe { rte_service_dump(stream, self.into()) } != 0)
		{
...
lazy_static!
{
	static ref HorribleHackToManageLifetimeOfRustRegisteredServices: Mutex<HashMap<Service, Box<Any>>> = Mutex::new(HashMap::with_capacity(64));
}
...
impl Service
{
	...
	pub fn register<F: 'static + ServiceFunction>(name: &[u8], function: Box<F>, numa_node_choice: NumaNodeChoice) -> Self
	{
		...
		let mut hack = HorribleHackToManageLifetimeOfRustRegisteredServices.lock().unwrap();
		...
		(&mut service_specification.name[0 .. name.len()]).copy_from_slice(name);
		...
		if likely(result == 0)
		{
			if unlikely(unsafe { rte_service_component_runstate_set(service_identifier, 1) } != 0)
			{
	...
	{
		if unlikely(unsafe { rte_service_lcore_reset_all() } != 0)
		{
	...
	{
		let mut service_identifer = unsafe { unintialized() };
	...
	pub fn enable_running_on_service_logical_core(self, logical_core: LogicalCore)
	{
		...
		if unlikely(unsafe { rte_service_map_lcore_set(self.into(), logical_core.into(), 1) } != 0)
		{
	...
	{
		if unlikely(unsafe { rte_service_runstate_set(self.into(), 1) } != 0)
		{
	...
	{
		if unlikely(unsafe { rte_service_runstate_set(self.into(), 0) } != 0)
		{
			panic!("rte_service_runstate_set failed")
		}
	}
	...
	pub fn disable_running_on_service_logical_core(self, logical_core: LogicalCore)
	{
		...
		if unlikely(unsafe { rte_service_map_lcore_set(self.into(), logical_core.into(), 0) } != 0)
		{
			panic!("rte_service_map_lcore_set failed")
		}
	}
	...
	{
		if unlikely(unsafe { rte_service_set_stats_enable(self.into(), 1) } != 0)
		{
	...
	{
		if unlikely(unsafe { rte_service_set_stats_enable(self.into(), 0) } != 0)
		{
			panic!("rte_service_set_stats_enable failed")
		}
	}
	...
	/// Resets statistics.
	#[inline(always)]
	pub fn reset_statistics(self) -> u32
	{
		if unlikely(unsafe { rte_service_attr_reset_all(self.into()) } == 0)
		{
	...
	fn get_attribute(self, attribute_identifier: u32) -> u32
	{
		...
		if likely(unsafe { rte_service_attr_get(self.into(), attribute_identifier, &mut value) } == 0)
		{
			value
		}
		else
