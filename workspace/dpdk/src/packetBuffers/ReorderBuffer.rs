...
impl ReorderBuffer
{
	...
	#[inline(always)]
	pub fn new(name: &CStr, numa_socket_id: Option<NumaSocketId>, maximum_number_of_elements: u32) -> Option<Self>
	{
		let result = unsafe { rte_reorder_create(name.as_ptr(), numa_socket_id.to_u32(), maximum_number_of_elements) };
		if result.is_null()
		{
			None
		}
