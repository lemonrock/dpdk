...
pub trait PacketBufferExt: PrintInformation
{
	...
	#[inline(always)]
	fn stripped_vlan_qinq_tag_control_information(self) -> (u16, u16)
	{
		(self.reference().vlan_tci_outer, self.stripped_vlan_tag_control_information())
	}
	...
	#[inline(always)]
	fn stripped_vlan_tag_control_information(self) -> u16
	{
		self.reference().vlan_tci
	}
	...
	#[inline(always)]
	fn ethernet_header(self) -> NonNull<eth_hdr>
	{
		// ?
		self.start_of_data::<eth_hdr>()
	...
	/// Will never return 0.
	#[inline(always)]
	fn number_of_fragmented_internet_protocol_version_6_packets_rounded_up(self, maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header: MaximumTransmissionUnitSizeInBytes) -> usize
	{
		let fragment_size = maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header.internet_protocol_version_6_fragment_size() as usize;
	...
	#[inline(always)]
	fn fragment_internet_protocol_version_6_packet(self, maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header: MaximumTransmissionUnitSizeInBytes, direct_packets_pool: Non<rte_mempool>, indirect_packets_pool: Non<rte_mempool>, fragments: &mut Vec<Self>)
	{
		debug_assert_ne!(maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header, 0, "maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header can not be zero");
		debug_assert!(maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header.as_u16() as usize > size_of::<ipv6_hdr>(), "maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header '{}' is not greater than internet protocol version 6 header", maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header);
		
		if self.length() >= maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header.as_usize()
		{
			fragments.push(self);
			return
		}
		...
		let number_of_fragments_required = self.number_of_fragmented_internet_protocol_version_6_packets_rounded_up(maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header);
		...
		let result = unsafe { rte_ipv6_fragment_packet(self.as_ptr(), packets_out, number_of_fragments_required as u16, maximum_transmission_unit_in_bytes_including_internet_protocol_version_6_header.as_u16(), direct_packet_buffer_pool.as_ptr(), indirect_packet_buffer_pool.as_ptr()) };
	...
	fn raw_free(self)
	{
		unsafe { rust_rte_mbuf_raw_free(self.as_ptr()) }
	}
   ...
   /// If the `length` is greater than the length of the first segment, the function will return an error and will not have modified the packet.
	...
	#[inline(always)]
	fn find_segment_at_offset(self, offset: u32) -> (NonNull<rte_mbuf>, u32)
	{
...
fn read_even_if_non_contiguous<'a>(self, length: u32, offset: u32, buffer: &'a mut [u8]) -> &'a mut [u8]
{
	...
	debug_assert(!pointer_to_read_from.is_null(), "pointer_to_read_from is null");
