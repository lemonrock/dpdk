...
pub struct InefficientProducer;

impl Producer for InefficientProducer
{
	// Producers one object.
	#[inline(always)]
	fn producer(ring: *mut rte_ring, object: *mut c_void) -> Result<(), ProducerError>
	{
		Self::processProducerResult(unsafe { rust_rte_ring_enqueue(ring, object) })
	}
	...
	{
		Self::processProducerResult(unsafe { rust_rte_ring_enqueue_bulk(ring, table, count) })
	}
	...
	{
		unsafe { rust_rte_ring_enqueue_burst(ring, table, count) }
	}
}
