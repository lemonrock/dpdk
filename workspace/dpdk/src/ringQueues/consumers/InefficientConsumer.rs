...
pub struct InefficientConsumer;

impl Consumer for InefficientConsumer
{
	...
	{
		Self::processConsumerResult(unsafe { rust_rte_ring_dequeue(ring, intoConsumerdObjectHolder) })
	}
	...
	{
		Self::processConsumerResult(unsafe { rust_rte_ring_dequeue_bulk(ring, table, count) })
	}
	...
	{
		unsafe { rust_rte_ring_dequeue_burst(ring, table, count) }
	}
}
