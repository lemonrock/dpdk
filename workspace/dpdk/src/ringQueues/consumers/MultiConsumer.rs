...
impl Consumer for MultiConsumer
{
	...
	{
		Self::processConsumerResult(unsafe { rust_rte_ring_mc_dequeue(ring, intoConsumerdObjectHolder) })
	}
	...
	{
		Self::processConsumerResult(unsafe { rust_rte_ring_mc_dequeue_bulk(ring, table, count) })
	}
	...
	{
		unsafe { rust_rte_ring_mc_dequeue_burst(ring, table, count) }
	}
}
