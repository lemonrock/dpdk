...
impl IpPacketReassemblyTable
{
	...
	#[inline(always)]
	pub fn create(numberOfBuckets: u16, entriesPerBucket: PowerOfTwoSixteenBit, maximumFlowTimeToLiveInMilliseconds: u64, numa_socket_id: Option<NumaSocketId>) -> Option<Self>
	{
		assert_ne!(numberOfBuckets, 0, "numberOfBuckets can not be zero");
		...
		Self::new(numberOfBuckets as u32, unsafe { PowerOfTwoThirtyTwoBit::from_u32_unchecked(entriesPerBucket) }, maximumEntries, maximumTimeToLiveInCyclesForEachFragmentedPacket, numa_socket_id, deathRowPreFetchFactor)
	}

	/// Please see the IMPORTANT NOTICE above which explains why this language has been used
	#[inline(always)]
	fn new(numberOfBuckets: u32, entriesPerBucket: PowerOfTwoThirtyTwoBit, maximumEntries: u32, maximumTimeToLiveInCyclesForEachFragmentedPacket: u64, numa_socket_id: Option<NumaSocketId>, deathRowPreFetchFactor: u8) -> Option<Self>
	{
		assert_ne!(numberOfBuckets, 0, "numberOfBuckets can not be zero");
		assert_ne!(maximumTimeToLiveInCyclesForEachFragmentedPacket, 0, "maximumCycles can not be zero");
		...
		let ipV4FragmentationTable = unsafe { rte_ip_frag_table_create(numberOfBuckets, bucketEntriesAsU32, maximumEntries, maximumTimeToLiveInCyclesForEachFragmentedPacket, numa_socket_id.as_c_int()) };
