...
impl TransmitQueue
{
	...
	pub fn new<Q: QueueMemoryConfiguration>(ethernetPortInformation: &EthernetPortInformation, queueIdentifier: QueueIdentifier, queueMemoryConfiguration: &Q, transmitQueueConfiguration: &TransmitQueueConfiguration, failures: &mut EthernetPortConfigurationFailures) -> Option<TransmitQueue>
	{
		...
		if likely(result == 0)
		{
			let mut transmitQueue = TransmitQueue
			{
				...
				numaSocketId: transmitQueueDescriptorsDmaMemoryAllocatedFromNumaSocketId,
	...
	pub fn startIfDeferred(&self) -> Result<(), ()>
	{
		...
		let result = unsafe { ::dpdk_sys::rte_eth_dev_tx_queue_start(self.portIdentifier, self.queueIdentifier) };
		if likely(result == 0)
		{
			Ok(())
		}
	...
	pub fn stop(&self)
	{
		let result = unsafe { ::dpdk_sys::rte_eth_dev_tx_queue_stop(self.portIdentifier, self.queueIdentifier) };
		if likely(result == 0)
		{
			()
		}
	...
	{
		let result = unsafe { ::dpdk_sys::rte_eth_dev_set_tx_queue_stats_mapping(self.portIdentifier, self.queueIdentifier, index) };
		if likely(result == 0)
		{
			true
		}
	...
	{
		let result = unsafe { ::dpdk_sys::rte_eth_set_queue_rate_limit(self.portIdentifier, self.queueIdentifier, maximumTransmissionRateInMbps) };
		if likely(result == 0)
		{
			Ok(())
		}
	...
	pub fn getInformation(&self) -> Result<rte_eth_txq_info, UnsupportedByHardwareError>
	{
		...
		let result = unsafe { ::dpdk_sys::rte_eth_tx_queue_info_get(self.portIdentifier, self.queueIdentifier, &mut information) };
		if likely(result == 0)
		{
			Ok(information)
		}
