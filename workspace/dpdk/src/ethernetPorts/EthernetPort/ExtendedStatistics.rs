...
impl EthernetPort
{
	...
	{
		unsafe { ::dpdk_sys::rte_eth_xstats_reset(self.portIdentifier()) }
	}
	
	#[inline(always)]
	pub fn retrieveExtendedStatisticsNames(&self) -> ExtendedStatisticNames
	{
		let size = match unsafe { ::dpdk_sys::rte_eth_xstats_get_names(self.portIdentifier(), null_mut(), 0) }
		{
		...
		let result = unsafe { ::dpdk_sys::rte_eth_xstats_get_names(self.portIdentifier(), names.as_mut_ptr(), size as c_uint) };
		assert!(result == size as c_int, "Second call to rte_eth_xstats_get_names() returned weird result '{}' instead of size '{}'", result, size);
