...
impl Drop for IpV4LongestPrefixMatchTable
{
	...
	fn drop(&mut self)
	{
		unsafe { ::dpdk_sys::rte_lpm_free(self.0) };
	}
}
...
impl LongestPrefixMatchTable for IpV4LongestPrefixMatchTable
{
	...
	fn new(name: &str, maximumRules: u32, numberOfTable8sToAllocate: u32, numaSocketId: Option<NumaSocketId>) -> Option<Self>
	{
		...
		let result = unsafe { ::dpdk_sys::rte_lpm_create(cName.as_ptr(), numaSocketId.as_c_int(), &configuration) };
	...
	fn lookUp(&self, ipAddress: &Self::IpHostAddress) -> Option<NextHop>
	{
		...
		let result = unsafe { ::dpdk_sys::rust_rte_lpm_lookup(self.0, *ipAddress, &mut nextHop) };
	...
	fn addRule(&mut self, networkAddress: &Self::IpNetworkAddress, nextHop: NextHop) -> bool
	{
		...
		let result = unsafe { ::dpdk_sys::rte_lpm_add(self.0, *ipAddress, depth, nextHop) };
		if likely(result == 0)
		{
			true
		}
	...
	fn hasRule(&self, networkAddress: &Self::IpNetworkAddress) -> Option<NextHop>
	{
		...
		let result = unsafe { ::dpdk_sys::rte_lpm_is_rule_present(self.0, *ipAddress, depth, &mut nextHop) };
	...
	fn deleteRule(&mut self, networkAddress: &Self::IpNetworkAddress) -> bool
	{
		...
		let result = unsafe { ::dpdk_sys::rte_lpm_delete(self.0, *ipAddress, depth) };
		if likely(result == 0)
		{
			true
		}
	...
	{
		unsafe { ::dpdk_sys::rte_lpm_delete_all(self.0) }
	}
}
...
impl IpV4LongestPrefixMatchTable
{
	...
	pub fn find(name: &str) -> Option<*mut rte_lpm>
	{
		...
		let result = unsafe { ::dpdk_sys::rte_lpm_find_existing(cName.as_ptr()) };
		if result.is_null()
		{
			None
		} else {
