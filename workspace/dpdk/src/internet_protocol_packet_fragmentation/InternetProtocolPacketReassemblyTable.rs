...
impl Drop for InternetProtocolPacketReassemblyTable
{
	...
	fn drop(&mut self)
	{
		unsafe { rust_rte_ip_frag_table_destroy(self.fragmentation_table.as_ptr()) };
...
impl InternetProtocolPacketReassemblyTable
{
	...
	/// * We use 100 seconds.
	#[inline(always)]
	pub fn defaultish(numa_socket_id: Option<NumaSocketId>) -> Self
	{
		const NumberOfBuckets: u16 = 4094;
		const EntriesPerBucket: PowerOfTwoSixteenBit = PowerOfTwoSixteenBit::_16;
		const MaximumFlowTimeToLive: Seconds = Milliseconds(100);
		Self::create(NumberOfBuckets, EntriesPerBucket, MaximumFlowTimeToLive, numa_socket_id)
	}
	
	/// `number_of_buckets` is really the maximum number of active flows, ie, of sets of fragmented packets.
	#[inline(always)]
	pub fn create(number_of_buckets: u16, entries_per_bucket: PowerOfTwoSixteenBit, maximum_flow_time_to_live: Seconds, numa_socket_id: Option<NumaSocketId>) -> Option<Self>
	{
		...
		let result = unsafe { rte_ip_frag_table_create(number_of_buckets, entries_per_bucket, maximum_entries, maximum_time_to_live_in_cycles_for_each_fragmented_packet, numa_socket_id.as_c_int()) };
