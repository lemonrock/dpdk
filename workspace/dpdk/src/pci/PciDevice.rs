...
impl PciDevice
{
	...
	pub fn bind_all_devices(sys_path: &Path, indirect_pci_device_identifiers: HashMap<IndirectPciDeviceIdentifier, PciDriver>) -> Vec<Unbind>
	{
		let all_known_pci_drivers = PciDriver::all_known_pci_drivers(sys_path);
	...
	fn indirect_pci_device_identifiers_to_pci_devices(sys_path: &Path, mut indirect_pci_device_identifiers: HashMap<IndirectPciDeviceIdentifier, PciDriver>) -> HashMap<IndirectPciDeviceIdentifier, (PciDevice, PciDriver)>
	{
	...
	fn ensure_bound_to_dpdk_driver(&self, sys_path: &Path, pci_drivers: &HashSet<PciDriver>, dpdk_pci_driver_to_bind_to: &PciDriver) -> Option<PciDriver>
	{
		assert!(dpdk_pci_driver_to_bind_to.is_dpdk_driver(), "dpdk_pci_driver_to_bind_to {:?} isn't a DPDK driver", dpdk_pci_driver_to_bind_to);
	...
	#[inline(always)]
	fn pci_driver<'a>(&self, sys_path: &Path, pci_drivers: &'a HashSet<PciDriver>) -> Option<&'a PciDriver>
	{
