...
impl PciDriver
{
	/// New.
	#[inline(always)]
	pub fn new(value: &str) -> PciDriver
	{
		use self::PciDriver::*;
	...
	#[inline(always)]
	pub(crate) fn all_known_pci_drivers(sys_path: &Path) -> HashSet<PciDriver>
	{
	...
	{
		if let PciDriver::NonDpdk(_) = *self
		{
			true
		}
		else
	...
	#[inline(always)]
	pub(crate) fn is_pci_device_bound(&self, sys_path: &Path, pci_device_address: &PciDeviceAddress) -> bool
	{
	...
	#[inline(always)]
	pub(crate) fn bind_pci_device(&self, sys_path: &Path, pci_device_address: &PciDeviceAddress) -> io::Result<()>
	{
	...
	#[inline(always)]
	pub(crate) fn unbind_pci_device(&self, sys_path: &Path, pci_device_address: &PciDeviceAddress) -> io::Result<()>
	{
